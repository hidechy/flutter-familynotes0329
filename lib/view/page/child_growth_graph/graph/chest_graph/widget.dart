...
/// 身体発育曲線(胸囲)グラフ
class ChildChestGraph extends StatelessWidget {
  const ChildChestGraph({
    super.key,
    ...
    return LineChartData(
      ...
      lineBarsData: [
        if (bandGraphDataList.isNotEmpty) ...[
          // 平均
          LineChartBarData(
            spots: _buildChestPlot(isUpper: true),
            isCurved: true,
            barWidth: 0,
            color: IHSGraphStyle.mainColor,
          ...
          LineChartBarData(
            spots: _buildChestPlot(isUpper: false),
            isCurved: true,
            barWidth: 0,
            color: IHSGraphStyle.mainColor,
            dotData: FlDotData(show: false),
  ...
  // 胸囲
  List<FlSpot> _buildChestPlot({
    required bool isUpper,
  }) {
    final list = <FlSpot>[];
    bandGraphDataList.asMap().forEach((index, data) {
      final chest = isUpper ? data.maxChest : data.minChest;
      // y軸上の数値を計算
      final posY = (chest - startY) / yInterval;
      final spot = FlSpot(
